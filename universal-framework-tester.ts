...
class UniversalFrameworkTester {
  private readonly frameworkConfigs: FrameworkConfig[] = [
    {
      name: "elysia",
      displayName: "Elysia",
      directory: "/Users/fuguoqiang/Desktop/vafast/vafast-benchmarks/frameworks/elysia",
    ...
    },
    {
      name: "hono",
      displayName: "Hono",
      directory: "/Users/fuguoqiang/Desktop/vafast/vafast-benchmarks/frameworks/hono",
      startCommand: ["bun", "run", "src/index.ts"],
      port: 3001,
      testEndpoints: [
        { path: "/techempower/json", method: "GET", description: "JSON序列化测试" },
    ...
    },
    {
      name: "express",
      displayName: "Express",
      directory: "/Users/fuguoqiang/Desktop/vafast/vafast-benchmarks/frameworks/express",
      startCommand: ["bun", "run", "src/index.ts"],
      port: 3002,
      testEndpoints: [
        { path: "/techempower/json", method: "GET", description: "JSON序列化测试" },
    ...
    },
    {
      name: "koa",
      displayName: "Koa",
      directory: "/Users/fuguoqiang/Desktop/vafast/vafast-benchmarks/frameworks/koa",
      startCommand: ["bun", "run", "src/index.ts"],
      port: 3003,
      testEndpoints: [
        { path: "/techempower/json", method: "GET", description: "JSON序列化测试" },
    ...
    },
    {
      ...
      directory: "/Users/fuguoqiang/Desktop/vafast/vafast-benchmarks/frameworks/vafast-mini",
      startCommand: ["bun", "run", "src/index.ts"],
      port: 3004,
      testEndpoints: [
        { path: "/techempower/json", method: "GET", description: "JSON序列化测试" },
    ...
    },
    {
      name: "vafast",
      displayName: "Vafast",
      directory: "/Users/fuguoqiang/Desktop/vafast/vafast-benchmarks/frameworks/vafast",
      startCommand: ["bun", "run", "src/index.ts"],
      port: 3005,
      testEndpoints: [
        { path: "/techempower/json", method: "GET", description: "JSON序列化测试" },
  ...
  private async startFrameworkServer(config: FrameworkConfig): Promise<number> {
    ...
    return new Promise((resolve, reject) => {
      ...
      const checkServerReady = () => {
        const testEndpoint = config.testEndpoints[0];
        if (!testEndpoint) {
          reject(new Error(`No test endpoints defined for ${config.name}`));
          return;
        }

        http.get(`http://localhost:${config.port}${testEndpoint.path}`, (res) => {
          if (res.statusCode === 200) {
            const coldStartTime = performance.now() - coldStartBegin;
            console.log(`✅ ${config.displayName} 启动成功，冷启动时间: ${coldStartTime.toFixed(2)}ms`);
   ...
   * 执行性能测试
   */
  private async runPerformanceTest(config: FrameworkConfig, testDuration: number = 10): Promise<{
    totalRequests: number;
    averageLatency: number;
    minLatency: number;
    maxLatency: number;
    ...
    const sendConcurrentRequests = async () => {
      while (performance.now() < endTime) {
        try {
          // 随机选择测试端点
          const endpoint = config.testEndpoints[Math.floor(Math.random() * config.testEndpoints.length)];
   ...
   * 测试单个框架
   */
  async testFramework(frameworkName: string, testDuration: number = 10): Promise<PerformanceMetrics | null> {
  ...
  private printFrameworkResult(result: PerformanceMetrics): void {
    ...
    console.log(`   📊  延迟范围:       ${result.minLatency.toFixed(2)} - ${result.maxLatency.toFixed(2)} ms`);
  ...
  printComparisonReport(results: PerformanceMetrics[]): void {
    ...
    sortedByRps.forEach((result, index) => {
      ...
      console.log(`${medal} ${result.framework.padEnd(15)} ${result.requestsPerSecond.toFixed(2).padStart(8)} RPS`);
    });
    ...
    sortedByLatency.forEach((result, index) => {
      const medal = index === 0 ? "🥇" : index === 1 ? "🥈" : index === 2 ? "🥉" : `${index + 1}.`;
      console.log(`${medal} ${result.framework.padEnd(15)} ${result.averageLatency.toFixed(2).padStart(8)} ms`);
    });
    ...
    sortedByColdStart.forEach((result, index) => {
      const medal = index === 0 ? "🥇" : index === 1 ? "🥈" : index === 2 ? "🥉" : `${index + 1}.`;
      console.log(`${medal} ${result.framework.padEnd(15)} ${result.coldStartTime.toFixed(2).padStart(8)} ms`);
    });
    ...
    console.log(`   • 最低延迟: ${Math.min(...results.map(r => r.averageLatency)).toFixed(2)} ms`);
    console.log(`   • 最快冷启动: ${Math.min(...results.map(r => r.coldStartTime)).toFixed(2)} ms`);
    console.log(`${"=".repeat(80)}\n`);
  }
